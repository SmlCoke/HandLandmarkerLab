...
在所有未忽略 ROI 上输出 TP/FP/TN/FN、accuracy、precision、recall、F1、FPR 和 FNR。Val 可以扫 threshold；Test 配置关闭调阈值，必须使用在 Val 上冻结的值。
...
默认输出目录分别是 `${HAND_DATA_ROOT}/hand_landmarker_runs/v1/eval/val` 与 `${HAND_DATA_ROOT}/hand_landmarker_runs/v1/eval/test`，其中包含 `metrics.json` 和逐 ROI `predictions.jsonl`。
...
- 本次 Gold JSONL 的 `labels_sha256` 与实际 Hand 模型的 `hand_model_sha256`。

`metrics.json` 汇总上述指标和 `landmark_output_health`，并记录 Gold JSONL 路径/SHA-256、实际评估 backend、模型路径/SHA-256、配置路径/SHA-256 及 canonical 数据审计结果。逐 ROI 与汇总 provenance 让不同训练路线可以按同一 Gold 和模型字节复核；这些哈希不表示 Palm 或端到端整图评估。

## 2. 任意文件夹人工复核

编辑 `configs/infer.yaml` 的输入、checkpoint 和输出路径，然后运行：
