...
`national-final-geometry-eos_1.0-gate-r1` 的 geometry 正式训练、固定 ROI Val 和 EOS 1.0 历史 infer 已完成。HLMF 已发布 `0809-soar-enhance` 等人工确认的 true-negative dataset；最新版 HLMF 数据外层契约与 HLML 训练读取器兼容，文件夹推理也已切换到 EOS 2.0。下一目标是选定负样本成员并完成 multitask 训练与评估。
...
- HLMF 正式发布首个 EOS 2.0 dataset 后，以新 snapshot ID 运行 data audit，确认 manifest、Registry、HCF 0809 溯源字段和 `256×256` published ROI，再进入训练。
