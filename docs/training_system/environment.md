...
`make paths` 会打印 Makefile 内固定的数据根、curated ID、run ID、当前 phase 和转换校准配置。它们不再主要依赖 shell 临时环境变量。
...
- `inspect` 审计 geometry Train、Val、锁定 Test，并计算图片 SHA-256 与跨 split 泄漏。

第一次还没有 curated 快照时先运行 `make curate`；不要创建空 JSONL 来绕过 doctor/inspect。
...
服务器路径改变或开始新实验时，修改并提交 Makefile。命令行仍可做一次明确覆盖，例如：

```bash
make eval-val HAND_PRETRAIN_PHASE=multitask
```

但正式训练的数据根和 ID 应留在仓库版本中，避免只存在于某次 shell 会话。
...
- `make train` 拒绝覆盖既有 run；新实验修改 run ID，续训才使用 `training.resume_checkpoint`。
