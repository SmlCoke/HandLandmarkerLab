...
后续 multitask 明确使用完整已发布负样本集 `neg-eos_2.0-hcf0813-hp0.5`。为解决 Eos-2.1 正样本与 Eos-2.0 负样本 proposal variant 冲突，warehouse 将“正样本发布域”和“已发布负样本域”分别执行 variant 唯一性门禁；同一域内出现多个 proposal variant 仍失败。split、raw image、ROI ID、Registry、路径和图片解码门禁没有放宽。
...
- 当前没有已发布 hard dataset，`hard_datasets: []`；因此 multi-finetune 在完成 mining、CVAT 复核与 HLMF 发布前不启动。
...
本轮没有启动 geometry、multitask 或 multi-finetune 正式训练，没有改写既有 checkpoint/snapshot。服务器原仓库和 `DatesetFab`、`TrainFab`、`HLMF-Enhance` 均保持只读；所有数据审计、预导出与推理 smoke 产物位于独立临时目录，验收后清理。环境依赖未增加，`requirements.txt` 与 `environment.yml` 无需更新。
