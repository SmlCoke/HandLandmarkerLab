...
2026-08-07 对 `FullEnhance0801`、`FullEnhanceVal0801`、`eos-1.0` 运行 geometry 全量数据审计：Train 72,226、Val 5,091、Test 2,816，manifest、Registry、路径和 `256×256` 灰度 ROI 解码均无错误。Train 72,226 条均为 `pseudo/POS_RUNTIME`，geometry 配置已与其对齐为 100% `POS_RUNTIME` 严格抽样。按现有 `performer_cross_split: warn` 策略报告 peak/soar 跨 split 警告。
